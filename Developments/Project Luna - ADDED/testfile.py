...
# Config for loops
after_loop_voice_recognition = CTkLabel(root, text="")
after_loop2 = CTkLabel(root, text="")
...
def ping_all():
    for i in range(100, 200):
        os.system(f"ping -c 2 192.168.2.{str(i)}")


mac_lamp1 = '14:eb:b6:d2:29:38'
mac_lamp2 = '14:eb:b6:d2:97:1a'

ip_lamp1 = "192.168.2.103"
ip_lamp2 = "192.168.2.102"

output = subprocess.Popen(["arp", "-a"], stdout=subprocess.PIPE)
output_str = str(output.communicate())

test_mode = False
if input("Test Mode \n[Y or N]\n> ") == "Y":
    test_mode = True
else:
    output_list = output_str.split("wlan0\\n?")


def find_ip_by_mac(mac):
    usable_list = output_list.copy()

    for i in reversed(range(100, 200)):
        for x in (range(0, len(usable_list))):
            if f"(192.168.2.{i}) at <incomplete> on" == usable_list[x].strip():
                usable_list.pop(x)
                break

    lamps = list()
    for i in usable_list:  # goes through the list (contains only used ip addresses and junk)
        print(i)
        if mac in i:
            lamps.append(i.strip())

    for i in reversed(lamps):
        lamps.remove(i)
        lamps.append(i.split(r"wlan0\nL530.Speedport_W723_V_Typ_A_1_01_022"))

    lamps[0].pop(0)
    lamps.append(lamps[0][0])
    lamps.pop(0)

    print(lamps)
    iid = 0
    checker = math.ceil((len(lamps) / 2))
    for i in lamps:
        lamps.append(i.split(" ")[1].replace("(", "").replace(")", ""))
        lamps.pop(0)
        iid += 1
        if iid == checker:
            break
    ...
    if lamps == []:
        return ["couldnt find ip maybe try refresh arp tabel", "ERROR"]
    return lamps


if input("Do you want to refresh arp tabel? (this will take a few minutes)\n[Y or N]\n> ") == "Y":
    ping_all()

if test_mode is False:
    returned1 = find_ip_by_mac(mac_lamp1)
    returned2 = find_ip_by_mac(mac_lamp2)
else:
    returned1 = [ip_lamp1]
    returned2 = [ip_lamp2]

print("The ip of Lamp1 is >" + returned1[0])
print("The ip of Lamp2 is >" + returned2[0])

# [     ]   []   {   }   {}  \ @


bulb_chris_1 = PyL530.L530(returned1[0], email, password)
bulb_chris_2 = PyL530.L530(returned2[0], email, password)
...
def voice_recognition(first=False):
    def write_recording():
        global loop_count
        ...
        if skip is False:
            ...
            for i in voice_command_manipulation_list:
                print(f"{len(voice_command_manipulation_list)+1} - {id}")
                if id == len(voice_command_manipulation_list):
                    break
                if i == "Luna":
                    if id != 1:
                        for v in reversed(range(0, int(id-1))):
                            voice_command_manipulation_list.pop(v)
                    if len(voice_command_manipulation_list) < 1:
                        break
                    cmd = voice_command_manipulation_list
                    if len(cmd) >= 5:
                        print(cmd[1]+" "+cmd[2]+" "+cmd[3]+" "+cmd[4])
                        if cmd[1]+" "+cmd[2]+" "+cmd[3]+" "+cmd[4] == "turn on the lights":
                            bulb_switch(switch="on")
                        elif cmd[1]+" "+cmd[2]+" "+cmd[3]+" "+cmd[4] == "turn off the lights":
    ...
    after_loop_voice_recognition.after(int(1000*(duration+0.1)), lambda: (write_recording()))
    return
...
def settings():
    def save_settings():
        with open("Misc/settings.txt", "w") as f:
